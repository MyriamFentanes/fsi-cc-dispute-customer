...
INSERT INTO guifragment (code,widgettypecode,plugincode,gui,defaultgui,locked) VALUES ('login_form','login_form',NULL,'<#assign wp=JspTaglibs["/aps-core"]>
...
<#else>
	...
	<form action="<@wp.url/>" method="post" class="form-horizontal margin-medium-top "  >
		...
		<input type="hidden" name="returnUrl" value="${RequestParameters.returnUrl}" />
                 ...
                 <#else>
                   <@wp.pageWithWidget widgetTypeCode="fsi-greeting" var="homePageCards" listResult=false />
                <#if (homePageCards??) >
